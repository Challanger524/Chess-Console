...
const bool ChessPice::Rule(const Coord from, const Coord to, const grid_t &grid)
{
	...
	if (from.x == to.x) {
		if (from.y < to.y) for (int i = from.y + 1; i < to.y; i++) if (grid[i][from.x] != nullptr) return false;//check down
		else for (int j = from.y - 1; j > to.y; j--) if (grid[j][from.x] != nullptr) return false;//check up
	}
	
	//if something on the way(horizontally)
	if (from.y == to.y) {
		if (from.x < to.y) for (int i = from.x + 1; i < to.x; i++) if (grid[i][from.x] != nullptr) return false;//check  right
		else for (int j = from.x - 1; j > to.x; j--) if (grid[j][from.x] != nullptr) return false;//chek left
	}
...
const bool Pawn::Rule(const Coord from, const Coord to, const grid_t &grid)
{
	...
	if (dist(from.x, to.x) == 1 && dist(from.y, to.y) == 1) if (grid[from.y][from.x]->GetColr() != grid[to.y][to.x]->GetColr()) return true;//if diagnoal enemy beating |good|
...
const bool King::Rule(const Coord from, const Coord to, const grid_t &grid)
{
	...
	if (from.y == to.y && dist(from.x, to.x) == 2) 
	{
		Coord rook{-1,-1};
		...
		if (rook.x == 7) for (int i = from.x + 1; i < 7; i++) if (grid[from.y][i] != nullptr) return false;
